...
final class FirstRunSetupWindowController: NSWindowController, NSWindowDelegate {
    ...
    private func showVoiceStep() {
        ...
        detail.stringValue = "In ChatGPT Settings → Voice, set Voice chat hotkey to Control–Option–V. Start from a new empty chat or task, then let Hey Codex send that hotkey after it hears “Hey Codex.”"
    ...
    @objc private func testVoiceShortcut() {
        ...
        controller.testVoiceShortcut { [weak self] result in
            ...
            switch result {
            case .success:
                self.detail.stringValue = "Voice shortcut sent. Confirm that ChatGPT Voice opened, then choose Done."
