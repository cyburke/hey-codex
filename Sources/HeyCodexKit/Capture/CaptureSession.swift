...
public final class CaptureSession {
    ...
    /// Feed a frame while capturing; emits the utterance and returns to
    /// listening once the speaker endpoints or the safety cap is hit.
    public func feedWhileCapturing(_ frame: [Float]) {
        captured.append(contentsOf: frame)
        postFireCount += frame.count
        let vadEndpoint = vad.hasEndpointed(captured, sampleRate: Int(sampleRate))
